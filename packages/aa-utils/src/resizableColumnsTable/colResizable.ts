...
export default class ColResizable {
  ...
  createGrips() {
    ...
    domElmThList.forEach((domElmTh, index) => {
      ...
      let domElmHandle;
      if (!hasHandleContainer && this.domElmHandleContainer) {
      ...
      if (index === this.lastThIndex && !hasHandleContainer) {
        addClass(domElmHandle as HTML, 'last-handle');
      }
  ...
  syncGrips() {
    ...
    const iconHeight = this.domElmHandleContainer.querySelector(
